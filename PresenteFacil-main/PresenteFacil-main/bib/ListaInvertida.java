...
public class ListaInvertida {
  ...
  public static List<String> prepararTermos(String busca) {
    if (busca == null || busca.isEmpty())
      return new ArrayList<>();

    // Remove acentos
    String normalizada = Normalizer.normalize(busca, Normalizer.Form.NFD)
        .replaceAll("[^\\p{ASCII}]", "");

    // Remove caracteres não alfabéticos
    normalizada = normalizada.replaceAll("[^a-zA-Z0-9 ]", " ").toLowerCase();

    // Divide em palavras
    String[] palavras = normalizada.trim().split("\\s+");

    return new ArrayList<>(Arrays.asList(palavras));
  }
